...
class FavouritesList(Base):
    """
    Class FavouritesList creates a table "favourites_list" in database "vKinder_bot_db"
    """
    __tablename__ = 'favourites_list'
    id_favourites = sql.Column(sql.Integer, primary_key=True, autoincrement=True, nullable=False)
...
class VkUserPhoto(Base):
    ...
    # vk_user_id = sql.Column(sql.Integer, sql.ForeignKey('favourites_list.vk_user_id', ondelete='CASCADE'))
...
def check_if_match_exists(id_vk):
    """
    Checks if match already present in database (both black and favourites list)
    :param id_vk: int
    :return: Tuple
    """
    favourites_list = session.query(FavouritesList.vk_user_id).filter_by(vk_user_id=id_vk).first()
    black_list = session.query(BlackList.vk_user_id).filter_by(vk_user_id=id_vk).first()
    return favourites_list, black_list


def add_new_match_to_favourites(vk_user_id, bot_user_vk_id, first_name, last_name, vk_user_url):
    """
    Adds new match to favourites list in accordance with user's request
...
def delete_match_from_favourites_list(vk_id):
    """
    Deletes match from favourites list
    :param vk_id: int
    """
    new_entry = session.query(FavouritesList).filter_by(vk_user_id=vk_id).first()
...
def show_favourites_list(vk_user_id):
    """
    Shows all profiles from current bot user's favourites list
    :param id_: int
    :return: list of users from favourites
    """
    all_favourites = session.query(FavouritesList.vk_user_first_name,
                                  FavouritesList.vk_user_last_name,
                                  FavouritesList.vk_user_url,
                                  VkUserPhoto.photo_name
                                  ).join(VkUserPhoto,
                                         VkUserPhoto.vk_user_id == FavouritesList.vk_user_id).filter(
        vk_user_id == FavouritesList.bot_user_vk_id).all()
    return all_favourites


def show_black_list(vk_user_id):
    """
    Shows all profiles from current bot user's black list
    :param id_: int
